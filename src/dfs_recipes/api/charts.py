...
from dfs_recipes.utils.js_validator import JavaScriptValidator
...
async def generate_chart(request: Request, chart: ChartRequest, background_tasks: BackgroundTasks):
    try:
        ...
        is_safe, result = js_validator.validate_llm_javascript(data.javascript)

        if not is_safe:
            log.error(f'JavaScript validation failed: {result}')
            raise HTTPException(
                status_code=500,
                detail='Generated JavaScript code is not safe to execute.'
            )
